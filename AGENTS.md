...
## Structure
- `wsgi.py:1` — entrypoint (`app = create_app()`). `src/__init__.py:8` defines `create_app()` (resolve `config_by_name`, `from_object`, `db.init_app` + `migrate.init_app` + `limiter.init_app`, aviso se `TURNSTILE_SECRET_KEY` ausente, `import src.urls.models` para registrar `Url` no `metadata`, registers `src/urls/routes.py:5` `urls_bp` + `src/captcha/routes.py:5` `captcha_bp`, serve `frontend/` em `/assets` com `/` → `index.html`).
...
- `src/captcha/` — `service.py:7` `CaptchaService.verify` (Turnstile via urllib stdlib; bypass com warning se `TURNSTILE_SECRET_KEY` ausente = modo dev), `routes.py:5` `captcha_bp` (`GET /api/config` expõe só a site key pública).
- `frontend/` — estático puro (`index.html`, `styles.css`, `app.js` sem build): busca `/api/config`, renderiza o widget Turnstile explícito, `POST /urls/shorten` com `{url, captcha_token}`, trata 201/400/429/500. Mesma origem por padrão (Flask serve em `/`).
...
- `Dockerfile:12` `COPY src/ ./src/` + `COPY wsgi.py ./` + `COPY migrations/ ./migrations/` + `COPY docker_entrypoint.sh ./` com `ENTRYPOINT` (wait-for-db com retry via socket stdlib + `flask db upgrade` + `flask run`), `compose.yaml:1` só `  url_shortener` (`build: .`) contra o PG 18 nativo do WSL (cluster `18/main`, porta `5432`, `host-gateway` → `172.17.0.1` estável); credenciais via `env_file: src/.env`, `DB_HOST`/`DB_PORT`/`FLASK_ENV=production` em `environment` (precedem o env_file; app monta a URI das partes, sem `DATABASE_URL` fixa); `requirements.txt` only at root. `src/.env` holds segredos locais (`DB_*`, `TURNSTILE_SECRET_KEY`, gitignored via `.dockerignore:14`, not via `.gitignore`); `TURNSTILE_SITE_KEY` (pública) tem default no `compose.yaml`.
